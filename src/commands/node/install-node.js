...
function downloadAndInstallBinary(url, targetPath) {
    return new Promise((resolve, reject) => {
        https.get(url, (response) => {
            if (response.statusCode === 302 && response.headers.location && response.headers.location.includes('github')) {
...
function installMind() {
  const url =
    'https://github.com/Mind-chain/Msc-node/releases/download/v1.0.4/mind'
